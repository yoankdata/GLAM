...
import { Menu, X, MapPin, ShieldCheck, Gem } from 'lucide-react';
import type { Database } from '@/types/database.types';
import { formatPrice } from '@/lib/formatPrice';

// Type pour la carte prestataire
type Provider = Database['public']['Tables']['providers']['Row'];

// --- PROVIDER CARD ---
export const ProviderCard = ({ provider }: { provider: Provider }) => {
  const getBadgeInfo = (plan: string | null) => {
    if (plan === 'premium_annuel') {
      return {
        label: 'Premium',
        color: 'bg-[#D1A85A] text-white',
        icon: <Gem size={12} className="mr-1" />,
      };
    }
    if (plan === 'pro_annuel') {
      return {
        label: 'Vérifié',
        color: 'bg-[#D4A6A8] text-white',
  ...
  // Si l'image échoue -> placeholder local
  const imageOnError = (e: React.SyntheticEvent<HTMLImageElement>) => {
    e.currentTarget.onerror = null;
    e.currentTarget.src = '/images/providers/placeholder.jpg';
  };
    ...
    <Link href={`/prestataires/${provider.slug}`} className="group block h-full">
      <div className="bg-white rounded-[14px] overflow-hidden border border-[#E8D0C8] shadow-sm hover:shadow-md transition-all h-full flex flex-col">
        <div className="relative h-64 overflow-hidden bg-gray-100">
          ...
          {badge && (
            <div
              className={`absolute top-3 right-3 px-3 py-1 rounded-full text-xs font-bold uppercase flex items-center ${badge.color}`}
            >
              {badge.icon}
              {badge.label}
            </div>
        ...
        <div className="p-5 flex flex-col flex-grow">
          <div className="flex justify-between items-start mb-2">
            <h3 className="font-serif text-xl text-[#0A0A0A] font-bold">
              {provider.name}
            </h3>
          </div>
          ...
          <div className="flex flex-wrap gap-2 mb-4">
            {provider.specialties?.slice(0, 2).map((s, i) => (
              <span
                key={i}
                className="text-xs bg-[#FAF9F7] text-gray-600 px-2 py-1 rounded-md border border-[#E8D0C8]"
              >
                {s}
              </span>
            ))}
          ...
          <div className="mt-auto pt-4 border-t border-gray-100 flex items-center justify-between">
            ...
            <span className="font-bold text-[#D4A6A8]">
...
export const Navbar = () => {
  ...
  const isActive = (path: string) =>
    pathname === path
      ? 'text-[#D4A6A8]'
      : 'text-gray-600 hover:text-[#D4A6A8]';
    ...
    <nav className="sticky top-0 z-50 bg-white/90 backdrop-blur-md border-b border-[#E8D0C8]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-20">
          <Link
            href="/"
            className="font-serif text-2xl font-bold tracking-tighter text-[#0A0A0A]"
          >
          ...
          <div className="hidden md:flex items-center space-x-8">
            ...
            <Link
              href="/prestataires"
              className={`text-sm font-medium ${isActive('/prestataires')}`}
            >
              Prestataires
            </Link>
            <Link
              href="/devenir-prestataire"
              className="bg-[#D4A6A8] text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-[#c49294] transition-colors"
            >
              Devenir Prestataire
            </Link>
          </div>
      ...
      {isOpen && (
        <div className="md:hidden bg-white border-b border-[#E8D0C8] px-4 py-4 space-y-4">
          ...
          <Link
            href="/devenir-prestataire"
            className="block text-[#D4A6A8] font-bold"
            onClick={() => setIsOpen(false)}
          >
            Devenir Prestataire
          </Link>
    ...
    <div className="max-w-7xl mx-auto px-4 text-center md:text-left">
      <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
        <div className="col-span-1 md:col-span-2">
          <div className="font-serif text-2xl font-bold text-[#0A0A0A] mb-4">
            GLAM<span className="text-[#D4A6A8]">.CI</span>
          </div>
        ...
        <div>
          ...
          <ul className="space-y-2 text-sm text-gray-600">
            <li>
              <Link href="/prestataires">Trouver une pro</Link>
            </li>
            <li>
              <Link href="/devenir-prestataire">S&apos;inscrire</Link>
            </li>
          </ul>
        </div>
        <div>
